...
public class EditLessonDialog extends JDialog implements ActionListener{
	private JLabel label1,label2,label3,label4,label5;
	private JTextField text1,text2,text3,text4,text5;
	private JButton add;
	private Lesson lesson;
	public EditLessonDialog(Lesson less) {
		// TODO Auto-generated constructor stub
		lesson = new Lesson(less);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == add) {
			if(!text1.getText().equals("")) {
				lesson.setName(text1.getText());
			}
			if(!text2.getText().equals("")) {
				lesson.setCode(text2.getText());
			}
			if(!text3.getText().equals("")) {
				lesson.setTeacherID(Integer.parseInt(text3.getText()));
			}
			if(!text4.getText().equals("")) {
				lesson.setVahed(Integer.parseInt(text4.getText()));
			}
			if(!text5.getText().equals("")) {
				lesson.setLimit(Integer.parseInt(text5.getText()));
			}
			try {
				SQLight_DataBase.editLesson(lesson);
				this.dispose();
			} catch (Exception e1) {
